...
ICPath = '/home/mfocean/Documents/2-PhD/98-models/2-ROMS/9-simulations/new_domain/'
ICname = 'southernCCS1_IC.nc'
...
inputFileName ='hycom_'+year+month+day+'12'+'_t0'+hour+'.nc'
...
timeValues = num2date(timeInput[0]-12.,timeInput.units,calendar=timeInput.calendar) #when time is added 12 hours more.

print('Converting to my ocean time')
oceanTime = datetime(timeValues.year, timeValues.month, timeValues.day, timeValues.hour, dayofwk=0)
...
print('data needed from your grid loaded')
print('....')
...
print('Loading bottom values')
bottom_salinity = inputData.variables['salinity_bottom']
bottom_temperature = inputData.variables['water_temp_bottom']
bottom_u = inputData.variables['water_u_bottom']
bottom_v = inputData.variables['water_v_bottom']
...
print('Creating arrays for u and v')
dst_u = np.zeros((cs_r.shape[0],lon_rho.shape[0],lat_rho.shape[1]))
dst_v = np.zeros((cs_r.shape[0],lon_rho.shape[0],lat_rho.shape[1]))

print('interpolating ssh')
ssh,converged = fill(ssh[0,:,:], 0, 1, **kw)
ssh1=mp.interp(ssh,lonInput,latInput,lon_rho,lat_rho,checkbounds=False,masked=False,order=1)

ssh1 = ssh1 * mask_rho
dst_ssh = np.ma.MaskedArray(ssh1,mask_rho==0.)


print('Starting horizontal interpolation for 3d variables')
print('...')
...
for k in range(len(depthInput)):
    ...
    salinity1 = salinity[0,k,:,:]
    perc_fillValue = (np.sum(salinity1.data!=salinity1.fill_value) + 1)/(np.sum(salinity1.data==salinity1.fill_value) + 1)

    if perc_fillValue < 0.9:

        salinity1 = bottom_salinity[0,:,:]
        salty,converged = fill(salinity1,0,1,**kw)
        salty = mp.interp(salty,lonInput,latInput,lon_rho,lat_rho,checkbounds=False,masked=True,order=1)

        temperature1 = bottom_temperature[0,:,:]
        tempp,converged = fill(temperature1,0,1,**kw)
        tempp = mp.interp(tempp,lonInput,latInput,lon_rho,lat_rho,checkbounds=False,masked=True,order=1)

        u1 = bottom_u[0,:,:]
        uu,converged = fill(u1,0,1,**kw)
        uu = mp.interp(uu,lonInput,latInput,lon_rho,lat_rho,checkbounds=False,masked=True,order=1)

        v1 = bottom_v[0,:,:]
        vv,converged = fill(v1,0,1,**kw)
        vv = mp.interp(vv,lonInput,latInput,lon_rho,lat_rho,checkbounds=False,masked=True,order=1)
...
for x in range(lon_rho.shape[0]):
    for y in range(lat_rho.shape[1]):
        z_new = cs_r * h[x,y]
        dst_temp[:,x,y] = np.interp(z_new, np.flipud(-depthInput), np.transpose(temp[::-1,x,y]))
        dst_sal[:,x,y] = np.interp(z_new, np.flipud(-depthInput), np.transpose(sal[::-1,x,y]))

dst_sal = np.ma.masked_where(dst_sal==0.,dst_sal)
dst_temp = np.ma.masked_where(dst_temp==0.,dst_temp)

print('Interpolating vertically u and v')
u_velocity=np.array(u_arr)#*mask_rho
v_velocity=np.array(v_arr)#*mask_rho

for x in range(lon_rho.shape[0]):
    for y in range(lat_rho.shape[1]):
        z_new = cs_r * h[x,y]
        dst_u[:,x,y] = np.interp(z_new,np.flipud(-depthInput), np.transpose(u_velocity[::-1,x,y]))
        dst_v[:,x,y] = np.interp(z_new,np.flipud(-depthInput), np.transpose(v_velocity[::-1,x,y]))

# move back to u,v points
dst_u1 = round(100.,1)+(0.5 * (dst_u[:,:,:-1] + dst_u[:,:,1:])) 
dst_u1 = dst_u1 * mask_u
dst_u1 = np.ma.masked_where(dst_u1==0., dst_u1)
dst_u1 = dst_u1 - round(100.,1)

dst_v1 = round(100.,1) + (0.5 * (dst_v[:,:-1,:] + dst_v[:,1:,:]))
dst_v1 = dst_v1 * mask_v
dst_v1 = np.ma.masked_where(dst_v1==0., dst_v1)
dst_v1 = dst_v1 - round(100.,1)
...
print('Ubar computed... computing vbar now...')
...
print('Getting fill_value from the data')
ss = dst_sal[-1,:,:]
fill_value = ss.fill_value

# Creating Netcdf
print('Creating netCDF now...')
...
#time_units = 'hours since '+year+'-'+month+'-'+day+' '+hour+':00:00'    
